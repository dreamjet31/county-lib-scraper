...
(async () => {
    ...
    endDate.setTime(startDate.getTime() + (increments - 1) * oneDayInMs);
    ...
    while (endDate < today) {
        ...
        await page.keyboard.press('Backspace')
        await page.type('input[name="DateTo"]', formatDate(endDate));
        await page.click('input[name="DateFrom"]', { clickCount: 3 })
                    ...
                    for (let i = 0; i < rows.length; i++) {
                        ...
                        try {
                            ...
                            if (newTab) {
                                ...
                                await writeRecordToCsv(data, 'tulsa_output.csv');
                                await newTab.close();
                            }
        ...
        startDate.setTime(startDate.getTime() + increments * oneDayInMs);
        endDate.setTime(startDate.getTime() + (increments - 1) * oneDayInMs);
        fs.writeFileSync("startDate.txt", formatDate(startDate));
    }
