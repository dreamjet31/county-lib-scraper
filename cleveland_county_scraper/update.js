...
(async () => {
    ...
    const inputFile = fs.readFileSync('input-update.json', 'utf8');
    const input = JSON.parse(inputFile);

    const endDate = moment(); // Set endDate to today's date
    const increments = parseInt(input.increments, 10);
    const startDate = moment().subtract((increments), 'days');
    ...
    await page.waitForSelector('.ss-action.ss-action-form.ss-utility-box.ss-action-page-search.ui-link');
    const elements = await page.$$('.ss-action.ss-action-form.ss-utility-box.ss-action-page-search.ui-link');
    ...
    const localEndDate = moment(startDate).add(increments, 'days');
    await page.waitForSelector("input#field_RecDateID_DOT_StartDate");
    await page.type("input#field_RecDateID_DOT_StartDate", startDate.format('MM/DD/YYYY'));

    await page.waitForSelector("input#field_RecDateID_DOT_EndDate");
    await page.type("input#field_RecDateID_DOT_EndDate", localEndDate.format('MM/DD/YYYY'));
        ...
        for (let i = 0; i < pageNum; i++) {
            ...
            try {
                ...
                const data = await page.evaluate(() => {
                    ...
                    const grantorElements = granteesElement.querySelectorAll("tr")[1].querySelector('td').querySelectorAll('div')[1];
                    const granteeElements = granteesElement.querySelectorAll("tr")[2].querySelector('td').querySelectorAll('div')[1];
                    ...
                    const grantors = () => {
                        ...
                        if (ulElement) {
                            ...
                            let items = Array.from(ulElement.querySelectorAll('li'), li => li.innerText);
                            return items.join(', ');
                        }
                    ...
                    const legalElements = document.querySelectorAll('.selfservice-onecolumn.ss-utility-box-vertical.ss-listview.ui-listview.ui-listview-inset')[2].querySelectorAll('li');

                    let legal = ''
                    if (legalElements[0].innerText.includes('Legal')) {
                        let tdElement = legalElements[1].querySelectorAll('div')[1].querySelector('ul');
                        if (tdElement) {
                            // Get text of all 'li' items and join them with a comma
                            let items = Array.from(tdElement.querySelectorAll('li'), li => li.innerText);
                            legal = items.join(', ');
                        } else {
                    ...
                    return [documentType, documentNumber, recordingDate, numberPages, documentDate, grantor, grantee, legal];
                ...
                fs.appendFileSync('update.csv', csvData);
